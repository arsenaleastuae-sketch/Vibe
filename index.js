...
(function() {
  ...
  function ensureCocktailModal() {
    ...
    modal.setAttribute('aria-label', 'Cocktail details');
    ...
    modal.innerHTML = ''
      ...
      + '    <img class="cocktail-modal-image" src="" alt="Cocktail" loading="lazy">'
      + '  </div>'
    ...
    function open(data) {
      ...
      img.src = data.image || 'img/cocktails/cocktail-1.svg';
      img.alt = data.title || 'Cocktail';

      modal.querySelector('.cocktail-modal-title').textContent = data.title || '';
      modal.querySelector('.cocktail-modal-description').textContent = data.description || '';

      var meta = [];
      if (data.price && data.price !== '—') meta.push('Цена: ' + data.price);
      modal.querySelector('.cocktail-modal-meta').textContent = meta.join(' • ');

      var order = modal.querySelector('.cocktail-modal-order');
      order.href = data.orderUrl || '#';
    ...
    // Prevent touch and scroll events from reaching the viewer when modal is open.
    stopTouchAndScrollEventPropagation(modal);

    window.__cocktailModal = { open: open, close: close, overlay: overlay, modal: modal };
  ...
  function createInfoHotspotElement(hotspot) {
    ...
    // Open centered modal on click/tap
    btn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      ensureCocktailModal().open(hotspot);
    });
